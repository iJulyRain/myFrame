...
{
	debug(DEBUG, "===> IO using udp writen by li zhixian @ june 2015,1 <===\n");
}
...
* @return 成功返回9，失败返回-1 
*/
static int udp_init(object_t parent, void *info, object_buf_t read_buf, object_buf_t send_buf)
{
...
static int udp_poll(object_t parent, int timeout)
{
	struct pollfd fds[1];
	object_io_t oi = (object_io_t)parent;
...
static int udp_connect(object_t parent)
{
	...
	oi->fd = socket(AF_INET, SOCK_DGRAM, 0);
	if(oi->fd < 0)
	{
...
*/
static int udp_recv(object_t parent, char *buffer, int size)
{	
	int rxnum;
	object_io_t oi;
	struct sockaddr_in *saddr;
...
*/
static int udp_send(object_t parent, const char *buffer, const int size)
{
	int txnum;
	object_io_t oi;
	struct sockaddr_in *saddr;
...
int udp_readbuf(object_t parent, char *buffer, int size)
{
	return buffer_readbuf(parent, buffer, size); 
}

int udp_sendbuf(object_t parent, const char *buffer, const int size)
{
	return buffer_sendbuf(parent, buffer, size); 
}

/**
* @brief 注册 udp IO 操作接口集
*/
void register_udp_io_operations(void)
{
	...
	oi->_info	       = udp_info;
	oi->_init	       = udp_init;
	oi->_connect       = udp_connect;
    oi->_state		   = udp_state;
	oi->_close	       = udp_close;
	oi->_poll          = udp_poll;
	oi->_recv	       = udp_recv;
	oi->_send	       = udp_send;
	oi->_readbuf	   = udp_readbuf;
	oi->_sendbuf	   = udp_sendbuf;

	object_addend(&oi->parent, "udp", object_class_type_io);
}
